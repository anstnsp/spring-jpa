...
public class SessionUser {
  ...
  //[User클래스를 사용하지 않는 이유]
  //일단 User클래스를 그대로 사용하면 Failed to convert from type [java.lang.Object] to type [byte []] 에러가 발생함. 
  ...
  //면 직렬화 대상에 자식들까지 포함되니 성능 이슈，부수 효과가 발생할 확률이 높습니다. 
  //그래서 직렬화 기능을 가진 세션 Dto를 하나 추가로 만드는 것이 이후 운영 및 유지보수 때 많은 도움이 됩니다.
   
  // 이제 모든 시큐리티 설정 끄읕~ 로그인 테스트하러 가보자. 
}